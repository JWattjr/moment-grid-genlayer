...
import { formatGen, genLayerGameConfig, readGameRounds, type GameRoundRecord } from "@/lib/genlayer-game";

export function RoundsClient() {
  const [rounds, setRounds] = useState<GameRoundRecord[]>([]);
  const [error, setError] = useState("");
  useEffect(() => {
    void readGameRounds().then(setRounds).catch((caught) => setError(caught instanceof Error ? caught.message : "Unable to load rounds."));
  }, []);
