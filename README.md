...
[Play the StudioNet V3 game](https://moment-grid-genlayer.vercel.app) · [Inspect live consensus proof](https://moment-grid-genlayer.vercel.app/genlayer) · [Review deployment receipts](deployments/genlayer/studionet.json)
...
5. Review [Integrity](https://moment-grid-genlayer.vercel.app/integrity), the [StudioNet V3 deployment manifest](deployments/genlayer/studionet.json), and the [full game specification](docs/ONCHAIN_GAME.md).
...
### Preserved Bradbury deployments

The Bradbury V3 contracts and their Arsenal–Coventry round remain recorded in [deployments/genlayer/bradbury-v3.json](deployments/genlayer/bradbury-v3.json); Bradbury V2 contracts and positions remain in [deployments/genlayer/bradbury.json](deployments/genlayer/bradbury.json), and the Integrity page links to the preserved V2 release for recovery. New public play uses the StudioNet V3 contracts. An earlier QA round exposed a callback-order failure: the game reached settlement while the resolver record remained pending. V3 removes automatic callback dispatch, so consensus must persist first and a separate permissionless transaction sends it to the game.
...
[`MatchMomentResolver`](contracts/match_moment_resolver.py) is a reusable, application-neutral contract for one constrained football criterion. Its settled StudioNet records give reviewers an immediately inspectable consensus proof while the active game fixture remains open. It keeps its own network and address configuration, separate from the game deployment.
...
Open `http://localhost:3003`. The checked-in environment example points at the public StudioNet V3 game and the reusable StudioNet resolver proof. Never add a private key, account password, or provider secret to an environment file.
